...
object Basics {
    ...
    def artificialExample: String = {
      if (System.getenv("test").toLowerCase == "value") "found" else "not found"
    }
    ...
    def power: Byte => Int => Long = {
      b: Byte => x: Int => {
        def go(value: Long, acc: Int): Long = {
          if (acc > 0) go(value*x, acc-1)
          else value
        }
        go(value = 1, acc = b.toInt)
      }
    }
    ...
    val formatTaskFunction = formatNamedValue[List[String]]("y",_.mkString(", "))
    ...
    val allTupleBooleanBooleans: Set[(Boolean, Boolean)] = Set( (false,false) , (false,true) ,
      (true,false)  , (true, true) )

    //TODO
    // Question. Can we make a `Set` with all possible `Byte` values? `Double` values? `String` values?
    /// → Byte:     YES
    /// → Double:   YES, because Double has finite accuracy of approximation
    /// → String:   YES iff it is not recursive type.
    ...
    //Uncurried versions: GCD & LCM
    //Curried versions: GCD1 & LCM1

    def gcd(a: Int, b: Int): Int = gcd1(a)(b)

    def lcm(a: Int, b: Int): Int = a*b / gcd1(a)(b)
    ...
    /* Amelioration of intuition */

    val f: (Int,Int) => Int = _ - _
    val g: Int => Int => Int = { x:Int => y:Int => f(x,y)}
    val g1 = curry(f)
    val f1 = unCurry(g)
    ...
    def flip[A,B,C]: (A => B => C) => B => A => C = {   // ~ type level
      f => x => y => f(y)(x)                            // ~ \f x y -> f y x
    }

    /* Church Logic */

    def faux[A,B] = {x:A => y:B => x}
    def vrai[A,B] = {x:A => y:B => y}

    def not[A,B] = flip

    def egal[A,B,C]: (A => B => C) => (A => B => C) => Boolean = ???

    //TODO
    // Implement functions: egal(equal), or, and.
    // solve the problem with Generics
